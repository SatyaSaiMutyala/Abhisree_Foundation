...
//     ref_role = prefs.getString('ref_role');
//     print('Ref role ---> ${ref_role}');
...
class _MainState extends State<Mainscreen> {
  ...
  Widget _buildDonationCard(double width, double height) {
    return _buildCard(
      width: width,
      height: height,
      imagePath: 'assets/images/Png/HandHeart.png',
  ...
  Widget _buildVolunteerCard(double width, double height) {
    return _buildCard(
      width: width,
      height: height,
      imagePath: 'assets/images/Png/Users.png',
  ...
  Widget _buildEmployeeCard(double width, double height) {
    return _buildCard(
      width: width,
      height: height,
      imagePath: 'assets/images/Png/Users.png',
      title: 'Join as Co-Ordinator (freelancer)',
      description:
          'Get started with a free membership by completing the form and enjoy priority access to services.',
      buttonText: 'Become a Co-Ordinator',
      onPressed: () => Navigator.pushNamed(context, AppRoutes.employeeagrement),
      outlined: true,
    );
  }
  ...
  return Scaffold(
    body: isLoading
        ...
        : Container(
            ...
            child: Builder(
              builder: (_) {
                if (ref_role == 'volunteer') {
                  return Center(
                    child: SingleChildScrollView(
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          _buildVolunteerCard(width, height),
                        ],
                      ),
                    ),
                  );
                } else if (ref_role == 'employee') {
                  return Center(
                    child: SingleChildScrollView(
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          _buildEmployeeCard(width, height),
                        ],
                      ),
                    ),
                  );
                ...
                return SingleChildScrollView(
                  child: Column(
                    children: [
                      SizedBox(height: height * 0.08),
                      _buildDonationCard(width, height),
                      _buildVolunteerCard(width, height),
                      _buildEmployeeCard(width, height),
                      SizedBox(height: height * 0.04),
                    ],
